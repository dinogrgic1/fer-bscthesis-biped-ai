...
void makeArm()
{
    ...
    for (int i = 0; i < NUM; i++)
    {
        rlink[i].body = dBodyCreate(w.world);
        dBodySetPosition(rlink[i].body, x[i], y[i], z[i]);
        dMassSetZero(&mass);
        dMassSetCapsuleTotal(&mass, weight[i], 3, r[i], length[i]);
        dBodySetMass(rlink[i].body, &mass);
        rlink[i].geom = dCreateCapsule(w.space, r[i], length[i]);
        dGeomSetBody(rlink[i].geom, rlink[i].body);
    }

    // ジョイントの生成とリンクへの取り付け
    joint[0] = dJointCreateFixed(w.world, 0); // 固定ジョイント
    dJointAttach(joint[0], rlink[0].body, 0);
    dJointSetFixed(joint[0]);
    for (int j = 1; j < NUM; j++)
    {
        joint[j] = dJointCreateHinge(w.world, 0); // ヒンジジョイント
...
{
    Pcontrol();                // P制御
    dWorldStep(w.world, 0.01); // 動力学計算
    drawArm();                 // ロボットの描画
}

/*** ドロースタッフの設定 ***/
void setDrawStuff(World w)
{
    w.fn.version = DS_VERSION; // バージョン番号
    w.fn.start = &start;       // start関数
    w.fn.step = &simLoop;      // simLoop関数
    w.fn.command = &command;   // command関数
    w.fn.path_to_textures = "textures";
}
...
int main(int argc, char **argv)
{
    ...
    setDrawStuff(w);
    makeArm();                                      // アームの生成
    dsSimulationLoop(argc, argv, 640, 480, &w.fn); // シミュレーションループ
