...
static std::vector<Motor*> joint(25);         /* all the motors */
static std::vector<PositionSensor*> joint_sensors(25); /* associated_sensors */
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 25; i++) {
    joint[i] = robot->getMotor(joint_number_to_name(i));
    joint_sensors[i] = joint[i]->getPositionSensor();
   ...
   * controller program.
   */
  //left_touch = robot->getDeviceFromTag("left touch");
  //Device *left_touch = new Device("left touch");
  Device *left_touch = robot->getDevice("left touch");
  left_touch->enable(control_step);
  Device *right_touch = robot->getDevice("right touch");
